...
const KNOWN_BRANDS = {
  ...
  'snitch.co.in':    { category: 'Fashion & Apparel', subCategory: 'Men\'s Fashion', onlineOnly: true },
...
async function scrapeStoreLocatorWithBrowser(storeLocatorUrl) {
  const { scrapeStoreLocatorPage } = require('./fetch');
  const { scoreStoreResponse, extractCountFromScoredResponse } = require('./storeInterceptor');

  let scrapeResult;
  try {
    scrapeResult = await Promise.race([
      scrapeStoreLocatorPage(storeLocatorUrl),
      new Promise((_, reject) => setTimeout(() => reject(new Error('stealth scrape timeout')), 18000)),
    ]);
  } catch {
    return { count: 0, source: 'none' };
  }

  const { html: storeHtml, capturedApiData, page } = scrapeResult;

  try {
    // Priority 1: Check XHR-intercepted API responses (most accurate)
    if (capturedApiData && capturedApiData.length > 0) {
      let bestCount = 0;
      for (const captured of capturedApiData) {
        const score = scoreStoreResponse(captured.data);
        if (score > 0) {
          const count = extractCountFromScoredResponse(captured.data);
          if (count > bestCount) bestCount = count;
        }
      }
      if (bestCount > 0) return { count: bestCount, source: 'xhr_intercept' };
    }

    if (!storeHtml || storeHtml.length < 500) return { count: 0, source: 'none' };

    // Priority 2: Text-based count from stealth-rendered page
    const textCount = extractStoreCount(storeHtml);
    if (textCount > 0) return { count: textCount, source: 'text_extraction' };

    // Priority 3: JSON arrays in rendered page
    const jsonCount = countJsonArrayItems(storeHtml);
    if (jsonCount > 0) return { count: jsonCount, source: 'json_array' };

    // Priority 4: DOM element counting
    const elemCount = countStoreElements(storeHtml);
    if (elemCount > 0) return { count: elemCount, source: 'store_elements' };

    // Priority 5: Rendered items / direction links
