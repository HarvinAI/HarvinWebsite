...
async function detectOfflineStores(html, url, technologies, fetchPage, storeLocatorUrl, jsonLdStoreHint, browserFetch) {
  ...
  // Start Wikipedia lookup in parallel (takes 1-3s) — we'll use it for cross-validation
  ...
  // Check both main page and store locator page for known widget patterns
  const combinedHtml = storeLocatorHtml + '\n' + html;
  try {
    const widgetCount = await tryWidgetParsers(combinedHtml, fetchPage);
  ...
  if (storeLocatorHtml) {
    ...
    // Try text extraction from store locator page (most reliable — actual text like "33 STORES")
    const countFromLocator = extractStoreCount(storeLocatorHtml);
    if (countFromLocator > 0) {
      // Cross-validate with Wikipedia for geo-filtered pages ("106 stores near you")
    ...
    // Fall back to JSON arrays and DOM element counting
  ...
  // --- Priority 2: XHR Interception Engine (catches SPA store locators) ---
  // Use Puppeteer to intercept actual API calls on the store locator page
  if (storeLocatorUrl) {
    try {
      const interceptedCount = await Promise.race([
        detectStoresViaInterception(storeLocatorUrl),
        new Promise(resolve => setTimeout(() => resolve(0), 40000)),
      ]);
      if (interceptedCount > 0) return countToBand(interceptedCount);
    } catch {}
  }
...
async function tryCommonStorePages(url, fetchPage, browserFetch) {
  ...
  const storePaths = [
    '/stores', '/store', '/store.html', '/store-locator', '/locations', '/find-a-store',
    ...
    '/en-in/stores', '/en/stores', '/en-in/store-locator',
    '/special-feature/store-locator',
    ...brandStorePaths,
    ...localePrefixPaths,
  ];
  const aboutPaths = ['/about', '/about-us', '/about-us.html', '/company',
    '/en-in/about-us.html', '/en-in/about-us', '/en/about-us', '/en/about'];
  ...
  if (best === 0 && browserFetch) {
    const topPaths = ['/stores', '/store', '/store.html', '/store-locator', '/store-finder', '/locations', '/our-stores', '/find-a-store', '/pages/store-locator', '/pages/stores'];
  ...
  if (noStoreBizTypes.includes(category)) {
    ...
    try {
      ...
      offlineStores = await Promise.race([
        ...
        new Promise(resolve => setTimeout(() => resolve(timeoutFallback), 45000)),
      ]);
    } catch {
