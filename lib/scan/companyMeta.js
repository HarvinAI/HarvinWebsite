...
const KNOWN_BRANDS = {
  ...
  // Custom Tailoring
  'bombayshirts.com':{ category: 'Fashion & Apparel', subCategory: 'Custom Shirts', stores: '11-50', region: 'Global' },
  ...
  'snitch.co.in':    { category: 'Fashion & Apparel', subCategory: 'Men\'s Fashion', stores: '11-50', region: 'Global' },
  'snitch.com':      { category: 'Fashion & Apparel', subCategory: 'Men\'s Fashion', stores: '11-50', region: 'Global' },
  ...
  'urbanladder.com':  { category: 'Home & Living', subCategory: 'Furniture', stores: '11-50', region: 'India' },
  'sleepycat.in':     { category: 'Home & Living', subCategory: 'Mattresses & Sleep', onlineOnly: true },
  'wakefit.co':       { category: 'Home & Living', subCategory: 'Mattresses & Sleep', stores: '11-50', region: 'India' },
  ...
  // Outdoor & Sports
  'decathlon.in':     { category: 'Outdoor & Sports', subCategory: 'Sports Retail', stores: '100+', region: 'India' },
  'decathlon.com':    { category: 'Outdoor & Sports', subCategory: 'Sports Retail', stores: '100+', region: 'Global' },
  ...
  // Eyewear
  'lenskart.com':     { category: 'Fashion Accessories', subCategory: 'Eyewear', stores: '100+', region: 'India' },
  'johnjacobs.com':   { category: 'Fashion Accessories', subCategory: 'Eyewear', stores: '51-100', region: 'India' },
  'titaneyeplus.com': { category: 'Fashion Accessories', subCategory: 'Eyewear', stores: '100+', region: 'India' },
  'vincesmallworld.com':{ category: 'Fashion Accessories', subCategory: 'Eyewear' },
  // Watches
  'titan.co.in':      { category: 'Fashion Accessories', subCategory: 'Watches', stores: '100+', region: 'India' },
  'fastrack.in':      { category: 'Fashion Accessories', subCategory: 'Watches & Accessories', stores: '100+', region: 'India' },
  'fossil.com':       { category: 'Fashion Accessories', subCategory: 'Watches', stores: '100+', region: 'Global' },
  'casio.com':        { category: 'Fashion Accessories', subCategory: 'Watches', stores: '100+', region: 'Global' },
  'rolex.com':        { category: 'Fashion Accessories', subCategory: 'Luxury Watches', stores: '100+', region: 'Global' },
  // Bags & Luggage
  'samsonite.com':    { category: 'Fashion Accessories', subCategory: 'Luggage & Travel', stores: '100+', region: 'Global' },
  'americantourister.com':{ category: 'Fashion Accessories', subCategory: 'Luggage & Travel', stores: '100+', region: 'Global' },
  'wildcraft.com':    { category: 'Fashion Accessories', subCategory: 'Backpacks & Outdoor', stores: '100+', region: 'India' },
  'skybags.co.in':    { category: 'Fashion Accessories', subCategory: 'Bags & Luggage', stores: '100+', region: 'India' },
  'mokobara.com':     { category: 'Fashion Accessories', subCategory: 'Luggage & Travel', onlineOnly: true },
  // Health & Wellness
  'cultfit.com':      { category: 'Health & Wellness', subCategory: 'Fitness & Gym', stores: '100+', region: 'India' },
  'curefit.com':      { category: 'Health & Wellness', subCategory: 'Fitness & Gym', stores: '100+', region: 'India' },
...
const SUB_INDUSTRY_KEYWORDS = {
  ...
  'Home & Living': {
    ...
    'Hardware Store':     ['hardware', 'power tools', 'hand tools', 'drill', 'saw', 'plumbing', 'timber', 'lumber', 'building materials', 'home improvement', 'renovation', 'diy', 'fencing', 'roofing', 'flooring', 'paint', 'garden', 'landscaping', 'warehouse'],
  },
  'Health & Wellness': {
...
async function detectOfflineStores(html, url, technologies, fetchPage, storeLocatorUrl, jsonLdStoreHint, browserFetch) {
  ...
  const headerFooterLink = findStoreLocatorInHeaderFooter(html, url);

  // If no store/location link in header/footer → brand sells online only
  if (!headerFooterLink) {
  ...
  // Try fetching store locator page with axios first (fast)
...
function findStoreLocatorInHeaderFooter(html, baseUrl) {
  ...
  const STORE_TEXT_KEYWORDS = /\b(?:stores?|locations?|outlets?|showrooms?|branches?|find\s+(?:a\s+)?store|store\s+(?:locator|finder)|our\s+stores?|visit\s+us|where\s+to\s+buy|locate\s+us|find\s+us|retail\s+stores?|experience\s+(?:centre|center)s?)\b/i;
...
async function extractCompanyMeta({ url, html, headers, metaMap, technologies, fetchPage, browserFetch, forceRefresh }) {
  ...
  if (GLOBAL_SIGNALS.test(scopeText)) {
    region = 'Global';
  } else if (NATIONAL_SIGNALS.test(scopeText) || citiesMentioned >= 3) {
    // National brand → keep region as country name
  } else if (city && citiesMentioned <= 1 && !NATIONAL_SIGNALS.test(scopeText)) {
    // Single-city brand → region = city name
    region = city;
  }
  // Otherwise region stays as country name (India, US, etc.) for national brands
  ...
  const result = { category, subCategory, region, state, city, offlineStores, storeRawCount };
...
module.exports = { extractCompanyMeta, INDIA_STATES };
