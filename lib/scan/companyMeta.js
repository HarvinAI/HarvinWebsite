...
async function extractCompanyMeta({ url, html, headers, metaMap, technologies, fetchPage, browserFetch, forceRefresh }) {
    ...
    // Priority 3: Active scraping — check header/footer for store link, follow it, count locations
      ...
      try {
        ...
        if (typeof storeResult === 'string') {
          ...
          // If it returns a low count, it's likely wrong (picked up a phone number, address, etc.)
          // Only trust it for high counts (50+) which are harder to false-positive.
          const source = storeResult.source;
          if (source === 'text_extraction' && storeRawCount < 50) {
  ...
  const result = { category, subCategory, region, state, city, offlineStores, storeRawCount, businessModel, categoryConfidence };
