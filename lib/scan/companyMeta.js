...
async function extractCompanyMeta({ url, html, headers, metaMap, technologies, fetchPage, browserFetch, forceRefresh }) {
  ...
  const appScanPromise = needFullAppScan
    ? Promise.race([
  ...
  // ── Business Model Detection ──
  // Prefer Gemini's read (best signal); fall back to the tech/store heuristic.
  let businessModel = aiBusinessModel || detectBusinessModel(html, url, offlineStores, technologies);
  // Final fallback: derive from store count if still unknown.
  if (!businessModel) {
