...
async function detectOfflineStores(html, url, technologies, fetchPage, storeLocatorUrl, jsonLdStoreHint, browserFetch) {
  ...
  // --- Priority 2: DeepSeek AI analysis (fast, accurate, works on any HTML) ---
  const contentForAI = storeLocatorHtml || html;
  if (contentForAI && contentForAI.length > 200) {
    try {
      const { getStoreCountFromAI } = require('./deepseekStoreCount');
      const aiCount = await getStoreCountFromAI(contentForAI, url);
      if (aiCount > 0) return result(aiCount, 'deepseek_ai');
    } catch {}
  }

  // --- Priority 3: If axios was blocked OR got no useful data, use stealth browser ---
  ...
  // --- Priority 4: Analyze axios-fetched store locator page ---
  if (storeLocatorHtml) {
    ...
    // Priority 3: Active scraping for store count
      ...
      try {
        ...
        const storeResult = await Promise.race([
          ...
          new Promise(resolve => setTimeout(() => resolve(timeoutFallback), 3000)),
        ]);
