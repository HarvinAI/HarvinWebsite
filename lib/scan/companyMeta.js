...
async function extractCompanyMeta({ url, html, headers, metaMap, technologies, fetchPage, browserFetch, forceRefresh }) {
  ...
  // Detect app presence from page HTML, with fallback to full domain scan
  const { detectAppPresenceFromHTML, detectAppPresence } = require('./appPresence');
  let appDetection = detectAppPresenceFromHTML(html);
  // If HTML-only detection found nothing or only one platform, try full domain scan
  if (appDetection.appPresence === 'No App' || appDetection.appPresence === 'iOS Only' || appDetection.appPresence === 'Android Only') {
    try {
      const fullScan = await detectAppPresence(normalizedDomain);
      if (fullScan.appPresence === 'Both iOS & Android' ||
          (fullScan.appPresence !== 'No App' && appDetection.appPresence === 'No App')) {
        appDetection = fullScan;
      }
    } catch {
      // Full scan failed, keep HTML-only result
    }
  }
...
module.exports = { extractCompanyMeta, INDIA_STATES, INDIA_CITY_STATE, CITY_ALIASES, normalizeCity, formatDisplayLocation, lookupKnownBrand, D2C_ELIGIBLE_CATEGORIES, analyzeKeywords, extractFromMeta };
