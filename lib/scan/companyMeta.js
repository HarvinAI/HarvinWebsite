...
const SUB_INDUSTRY_KEYWORDS = {
  ...
  'Health & Wellness': {
    ...
    'Sports Nutrition':   ['sports nutrition', 'whey protein', 'bcaa', 'creatine', 'pre-workout', 'post-workout', 'mass gainer', 'protein shake', 'electrolyte', 'energy gel'],
  ...
  'Grocery & Supermarket': {
    ...
    'Meat & Seafood':     ['meat shop', 'butcher', 'seafood market', 'fresh fish', 'halal meat', 'chicken shop', 'frozen meat', 'cold cuts'],
  ...
  'Dating & Matchmaking': {
    ...
    'Matrimony':          ['matrimony', 'matrimonial', 'shaadi', 'vivah', 'rishta', 'bride', 'groom', 'biodata', 'horoscope matching', 'kundli matching'],
...
async function extractCompanyMeta({ url, html, headers, metaMap, technologies, fetchPage, browserFetch, forceRefresh }) {
  ...
  try {
    ...
    // Check if domain is validated as Non-D2C — preserve that flag during rescans
    isNonD2C = existing?.category === 'Not Required' && existing?.d2cValidation?.isD2C === false;
    if (isNonD2C) {
      nonD2CReason = existing?.d2cValidation?.reason || 'This is not a D2C brand';
    }
    ...
    if (isNonD2C) {
      ...
      // Auto-compute Harvin Score on every scan
      const mv = existing?.monthlyVisits || 0;
      const tc = Array.isArray(technologies) ? technologies.length : 0;
      const app = result.appPresence || 'No App';
      const bm = result.businessModel || 'Pure D2C';
      const conf = result.categoryConfidence || 'low';
      const st = result.offlineStores || 'Online';
      let hs = 0;
      if (mv >= 20000000) hs += 30; else if (mv >= 5000000) hs += 25; else if (mv >= 1000000) hs += 20;
      else if (mv >= 500000) hs += 15; else if (mv >= 200000) hs += 10; else if (mv >= 50000) hs += 5; else if (mv >= 10000) hs += 2;
      if (tc >= 20) hs += 20; else if (tc >= 15) hs += 15; else if (tc >= 10) hs += 10; else if (tc >= 5) hs += 5; else if (tc >= 1) hs += 2;
      if (app === 'Both iOS & Android') hs += 15; else if (app !== 'No App') hs += 8;
      if (st === '500+' || st === '100+') hs += 15; else if (st === '100-500' || st === '51-100') hs += 12;
      else if (st === '50-100' || st === '21-50') hs += 9; else if (st === '10-50' || st === '11-20') hs += 6; else if (st === '1-10') hs += 3;
      if (conf === 'high') hs += 10; else if (conf === 'medium') hs += 6; else hs += 2;
      if (bm === 'Omnichannel') hs += 10; else if (bm === 'D2C + Marketplace') hs += 8; else if (bm === 'D2C + B2B') hs += 6; else hs += 5;
      setFields.harvinScore = hs;
    }
