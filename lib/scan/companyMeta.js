...
const KNOWN_BRANDS = {
  ...
  'foxtale.in':       { category: 'Beauty & Personal Care', subCategory: 'Skincare' },
  ...
  if (hasStoreSignal) {
    ...
    if (storeLocatorUrl && browserFetch) {
      try {
        ...
        if (browserCount > 0) return result(browserCount, 'browser_scrape');
      } catch {}
    }
  }
...
async function extractCompanyMeta({ url, html, headers, metaMap, technologies, fetchPage, browserFetch, forceRefresh }) {
  ...
  try {
    ...
    if (existing?.overrides) {
      ...
      if (existing.overrides.offlineStores) {
