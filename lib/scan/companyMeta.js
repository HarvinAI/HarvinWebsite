...
const INDUSTRY_KEYWORDS = {
  ...
  'Luxury & Premium':  ['luxury brand', 'premium collection', 'haute couture', 'bespoke', 'handcrafted luxury', 'exclusive collection', 'limited edition luxury', 'luxury lifestyle', 'concierge', 'private label luxury', 'atelier', 'maison'],
...
function analyzeKeywords(html, url, extraMeta) {
  ...
  const domainRaw = (url || '').replace(/^https?:\/\//i, '').replace(/^www\./i, '').replace(/\..*$/, '').toLowerCase();
  // Decompose compound domain names: "sydneytools" → "sydney tools", "mochishoes" → "mochi shoes"
  const DOMAIN_WORDS = ['shop','store','tools','shoes','fashion','beauty','tech','home','food','baby','pet','wear','mart','hub','gear','zone','world','box','club','fit','health','sport','sports','auto','cars','bike','book','books','watch','watches','jewel','decor','craft','art','game','games','music'];
  let domainName = domainRaw;
  for (const w of DOMAIN_WORDS) {
    if (domainRaw.length > w.length && domainRaw.includes(w) && domainRaw !== w) {
      domainName = domainRaw.replace(new RegExp(w, 'g'), ` ${w} `).replace(/\s+/g, ' ').trim();
    }
  }
