...
async function scanSingleUrl(rawUrl, { forceRefresh = false, pageData = null, metaOnly = false } = {}) {
  ...
  const techsPromise = (async () => {
    // Restrict tech detection to the <head> only (see extractHead above).
    // Re-derive scripts + meta from the head slice so nothing from <body>,
    // response headers, cookies, JS globals or network requests leaks in.
