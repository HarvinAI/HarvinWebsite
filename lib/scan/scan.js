...
async function scanSingleUrl(rawUrl, { forceRefresh = false } = {}) {
  ...
  // Merge sitemap data into html for keyword analysis if main page was blocked
  const analysisHtml = stillBlocked && sitemapHtml.length > 50
    ? `<title>${html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1] || ''}</title><body>${sitemapHtml}</body>`
    : html;
