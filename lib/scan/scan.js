...
async function scanSingleUrl(rawUrl, { forceRefresh = false, pageData = null } = {}) {
  ...
  if (pageData) {
    // ── Extension provided pre-captured page data — skip all server-side fetching ──
    // The extension runs in the user's real browser: no bot blocking, full DOM,
    // cookies, JS globals, and all network resource URLs from performance API.
    html       = pageData.html || '';
    ...
    headers    = {}; // extension can't capture HTTP response headers
  } else {
    // ── Server-side fetch (dashboard, API callers) ──
    try {
  ...
  // (skip when pageData is provided — extension HTML is the real rendered page)
  ...
  const stillBlocked = !pageData && (!html || html.length < 2000 ||
  ...
  // Use extension-provided metaMap if available (live DOM is more accurate than regex)
