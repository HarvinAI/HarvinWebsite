...
async function scanSingleUrl(rawUrl, { forceRefresh = false, pageData = null } = {}) {
  ...
  // Only use cache if it was written AFTER our classification improvements (March 2026)
  ...
  if (!forceRefresh) {
    try {
      ...
      if (cached && cached.expiresAt && new Date(cached.expiresAt) > new Date()
          && cached.updatedAt && new Date(cached.updatedAt) > CACHE_CUTOFF
          && cached.categoryConfidence && cached.categoryConfidence !== 'low'
