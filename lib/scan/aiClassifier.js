...
function buildPrompt({ domain, context, categories, lockedCategory, currentCategory }) {
  const catInstruction = lockedCategory
    ...
    : `Choose the single best category from this exact list:\n${categories.join(', ')}\n` +
      ...
      `Use "Unknown" only if classification is genuinely impossible.`;
...
- "subCategory" MUST be specific (2-4 words) describing the actual product line, e.g. "Soda", "Nail Care", "Men's Grooming", "Supplements", "Cookware", "Cold Brew Coffee".
